...
async function main() {
  ...
  console.log("Before buying ride");
  await cosoleBalances(addresses);

  const amount = { value: hre.ethers.utils.parseEther("1") };
  //console.log("Ayush"+ amount)
  await contract.connect(from1).buyRide("from1", "amount", amount);
  await contract.connect(from2).buyRide("from2", "amount", amount);
  await contract
    .connect(from3)
    .buyRide("from3", "amount", amount);

  console.log("After buying ride");
  await cosoleBalances(addresses);
